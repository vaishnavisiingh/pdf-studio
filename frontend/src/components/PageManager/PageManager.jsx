...
export default function PageManager({ docId, totalPages, onClose }) {
  ...
  const handleExtract = async () => {
    ...
    setLoading(true);
    try {
      const res = await fetch("http://127.0.0.1:8000/api/pages/extract", {
        method: "POST",
  ...
  const handleSplit = async () => {
    ...
    setLoading(true);
    try {
      const res  = await fetch("http://127.0.0.1:8000/api/pages/split", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
  ...
  const handleMerge = async () => {
    ...
    try {
      const res = await fetch("http://127.0.0.1:8000/api/pages/merge", {
        method: "POST",
        body: formData,
      });
      if (res.ok) {
