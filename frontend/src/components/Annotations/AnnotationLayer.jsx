...
export default function AnnotationLayer({ docId, page, width, height, activeTool, refreshKey }) {
  ...
  const fetchAnnotations = async () => {
    try {
      const res  = await fetch(`${import.meta.env.PROD ? "" : (import.meta.env.VITE_API_URL || "http://127.0.0.1:8000")}/api/annotations/${docId}/page/${page}`);
  ...
  const saveAnnotation = async (ann) => {
    try {
      const res  = await fetch(`${import.meta.env.PROD ? "" : (import.meta.env.VITE_API_URL || "http://127.0.0.1:8000")}/api/annotations/add`, {
        method: "POST",
  ...
  const deleteAnnotation = async (annId) => {
    try {
      await fetch(`${import.meta.env.PROD ? "" : (import.meta.env.VITE_API_URL || "http://127.0.0.1:8000")}/api/annotations/delete`, {
        method: "DELETE",
        headers: { "Content-Type": "application/json" },
