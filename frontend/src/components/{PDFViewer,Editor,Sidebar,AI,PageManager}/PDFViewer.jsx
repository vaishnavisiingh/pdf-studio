...
export default function PDFViewer({ docId, totalPages, externalPage, onPageChange, refreshKey, activeTool, pendingSignature, onSignaturePlaced, cropMode, onCropApplied }) {
  ...
  const handleMouseUp = async (e) => {
    if (isRedacting) {
      ...
      try {
        await fetch(`${import.meta.env.PROD ? "" : (import.meta.env.VITE_API_URL || "http://127.0.0.1:8000")}/api/redact/region`, {
          method: "POST",
    ...
    if (isDragging) {
      ...
      if (dragTool === "crop") {
        ...
        input.onchange = async (ev) => {
          ...
          try {
            await fetch(`${import.meta.env.PROD ? "" : (import.meta.env.VITE_API_URL || "http://127.0.0.1:8000")}/api/insert/${docId}/image?page=${currentPage}&x=${rect.x}&y=${rect.y}&width=${rect.width}&height=${rect.height}`, { method: "POST", body: formData });
            fetchPage(currentPage, Date.now());
  ...
  const handleTextInsert = async () => {
    ...
    try {
      await fetch(`${import.meta.env.PROD ? "" : (import.meta.env.VITE_API_URL || "http://127.0.0.1:8000")}/api/document/${docId}/insert-text?page=${currentPage}&x=${textPos.x}&y=${textPos.y}&text=${encodeURIComponent(textValue)}&fontsize=${fontSize}`, { method: "POST" });
  ...
  const handleTableInsert = async () => {
    ...
    try {
      await fetch(`${import.meta.env.PROD ? "" : (import.meta.env.VITE_API_URL || "http://127.0.0.1:8000")}/api/document/${docId}/insert-table?page=${currentPage}&x=${dragRect.x}&y=${dragRect.y}&rows=${tableRows}&cols=${tableCols}&cell_width=${cellW}&cell_height=${cellH}`, { method: "POST" });
  ...
  const handleEquationInsert = async () => {
    ...
    try {
      await fetch(`${import.meta.env.PROD ? "" : (import.meta.env.VITE_API_URL || "http://127.0.0.1:8000")}/api/document/${docId}/insert-equation?page=${currentPage}&x=${dragRect.x}&y=${dragRect.y + dragRect.height * 0.8}&equation=${encodeURIComponent(equationValue)}&fontsize=${fontsize}`, { method: "POST" });
