...
export default function FindReplace({ docId, onClose, onRefreshPage }) {
  ...
  const handleFind = async () => {
    ...
    setMessage(null);
    try {
      const res  = await fetch(`${import.meta.env.PROD ? "" : (import.meta.env.VITE_API_URL || "http://127.0.0.1:8000")}/api/search/${docId}?q=${encodeURIComponent(findText)}`);
  ...
  const handleReplace = async () => {
    ...
    setMessage(null);
    try {
      const res  = await fetch(`${import.meta.env.PROD ? "" : (import.meta.env.VITE_API_URL || "http://127.0.0.1:8000")}/api/search/${docId}/replace`, {
        method: "POST",
