...
const handleWatermark = async () => {
  if (!docId) return;
  try {
    const res = await fetch("http://127.0.0.1:8000/api/watermark/apply", {
      method: "POST",
...
const handleProtect = async () => {
  ...
  if (password.length < 4) {
    setPwdError("Minimum 4 characters required");
    return;
  }
  try {
    ...
    try {
      const res = await fetch("http://127.0.0.1:8000/api/repaginate/", {
        ...
        body: JSON.stringify({
          ...
          margin_bottom: marginTop,
          margin_left: marginLeft,
          margin_right: marginLeft,
...
<button
  className="repaginate-btn"
  style={{ background: "#d97706", marginRight: 6 }}
...
>
  ...
  <button
  className="repaginate-btn"
  style={{ background: "#7c3aed", marginRight: 6 }}
...
<button
  className="repaginate-btn"
  style={{ background: "#374151", marginRight: 6 }}
...
>
  ↓ TXT
        ...
        <button
          className="repaginate-btn"
          style={{ background: "#059669", marginRight: 6 }}
          onClick={() => { setShowFindReplace(!showFindReplace); setShowFontPanel(false); }}
        ...
        <button
          className="repaginate-btn"
          onClick={() => { setShowFontPanel(!showFontPanel); setShowFindReplace(false); }}
        >
        ...
        <button
  className="repaginate-btn"
  style={{ background: "#dc2626", marginRight: 6 }}
...
<button
  className="repaginate-btn"
  style={{ background: "#dc2626", marginRight: 6 }}
  onClick={() => { setShowPassword(!showPassword); setShowWatermark(false); setShowFontPanel(false); setShowFindReplace(false); }}
>
  🔒 Protect
</button>
        ...
        <div className="font-panel">
          ...
          <div className="font-panel-grid">
            ...
            <div className="font-field">
              ...
              <input type="number" min="0.5" max="2" step="0.1" value={marginTop}
                onChange={e => setMarginTop(Number(e.target.value))} />
            </div>
            <div className="font-field">
              <label>Left Margin</label>
              <input type="number" min="0.5" max="2" step="0.1" value={marginLeft}
                onChange={e => setMarginLeft(Number(e.target.value))} />
            </div>
            <div className="font-field">
              <label>Line Spacing</label>
              <input type="number" min="1" max="3" step="0.1" value={lineSpacing}
...
{showPassword && (
  <div className="font-panel">
    ...
    {pwdError && <div style={{ color: "#dc2626", fontSize: 12, marginBottom: 8 }}>{pwdError}</div>}
    <div className="font-panel-actions">
