...
export default function PageDecorPanel({ docId, totalPages, onClose, onComplete }) {
  ...
  const handlePageNumbers = async () => {
    setLoading(true);
    try {
      const res = await fetch(`${import.meta.env.PROD ? "" : (import.meta.env.VITE_API_URL || "http://127.0.0.1:8000")}/api/pagedecor/page-numbers`, {
        method: "POST",
  ...
  const handleHeaderFooter = async () => {
    ...
    setLoading(true);
    try {
      const res = await fetch(`${import.meta.env.PROD ? "" : (import.meta.env.VITE_API_URL || "http://127.0.0.1:8000")}/api/pagedecor/header-footer`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
  ...
  const handleRotate = async () => {
    ...
    try {
      const res = await fetch(`${import.meta.env.PROD ? "" : (import.meta.env.VITE_API_URL || "http://127.0.0.1:8000")}/api/rotate/apply`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
